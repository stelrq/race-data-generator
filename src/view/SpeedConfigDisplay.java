...
public class SpeedConfigDisplay extends GridPane {
	...
	// SpeedConfigDisplay
	public SpeedConfigDisplay(ParticipantSpeed participantSpeed, BiConsumer<Double, Double> theConsumer) {
		super();
		myBiConsumer = theConsumer;

		// Instantiation
		velocityField = new TextField(Double.toString(participantSpeed.getVelocity()));
		rangeField = new TextField(Double.toString(participantSpeed.getRange()));
		speedText = new Text(participantSpeed.toString());
		Text bracketText = new Text(participantSpeed.toString() + " Participant Settings");
		...
		setHgap(3);
		velocityField.setTooltip(new Tooltip("The base speed of the participants in this speed bracket class in track length units per millisecond"));
		rangeField.setTooltip(new Tooltip("The variation allowed in the velocity, participants randomly select a speed for each track section within\n"
	...
	class FieldChangeListener implements ChangeListener<String> {
		...
		@Override
		public void changed(ObservableValue<? extends String> obsVal, String oldString, String newString) {
			// We don't care about the new and old values, we know we want to try and send
			// the updated velocity and range regardless
			if (validDouble(velocityField.textProperty().get()) && validDouble(rangeField.textProperty().get())) {
				myBiConsumer.accept(Double.parseDouble(velocityField.textProperty().get()), Double.parseDouble(rangeField.textProperty().get()));
			}
		}
