...
//		});
	...
	public static void hackTooltipDurationTiming(Tooltip tooltip) {
		try {
			Field fieldBehavior = tooltip.getClass().getDeclaredField("BEHAVIOR");
			fieldBehavior.setAccessible(true);
			Object objBehavior = fieldBehavior.get(tooltip);
			...
			objTimer.getKeyFrames().clear();
			objTimer.getKeyFrames().add(new KeyFrame(new Duration(100000)));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	private void setToolTips() {
		// Call the hack to make tooltips last longer
		Tooltip hackToolTip = new Tooltip(
				"Used to change the amount of time it takes for a telemetry message to be generated for each racer\n"
		...
		hackTooltipStartTiming(hackToolTip);
		hackTooltipDurationTiming(hackToolTip);

		telemetryIntervalSlider.setTooltip(hackToolTip);

		numLapsField.setTooltip(new Tooltip("The number of laps to race for\n"
		...
		trackLengthField.setTooltip(new Tooltip("The distance of one lap of the race in arbitrary units\n"
				...
				+ "Larger number = racers moving slower and a longer race"));
		xRatioField.setTooltip(new Tooltip("The x Ratio of the track (how stretched it is horizontally)\n"
				+ "Changing this is the easiest way to change the shape of the track\n"
				+ "Larger number (for fixed yRatio) = track is stretched more left and right with smaller turns\n"
				+ "Smaller number = more rounded track with larger turns\n"
				+ "NOTE: xRatio should be greater than or equal to yRatio"));
		yRatioField.setTooltip(new Tooltip("The y Ratio of the track (how big vertically the track is)\n"
				+ "The best way to change the track shape is to modify the xRatio, but modifying the yRatio can result in fun new track shapes\n"
				+ "Larger number (for fixed xRatio) = taller track\n"
				+ "NOTE: xRatio should be greater than or equal to yRatio"));
		for (int i = 0; i < myTrackSectionComboBoxes.size(); i++) {
			myTrackSectionComboBoxes.get(i).setTooltip(new Tooltip("The speed of this section of track\n"
					+ "This determines the speed of participants on certain sections of the track relative to other sections\n"
					+ "NOTE: These are in the order that participants traverse them, \n"
					+ "they start right before the first turn, then travel to the second turn, then the bottom straight etc."));
		}
		for (int i = 0; i < trackSpeedMultiplierFields.size(); i++) {
			trackSpeedMultiplierFields.get(i).setTooltip(new Tooltip("The multiplier for this track section speed\n"
					+ "These correspond with the sections of track, and is different than participant speed classes\n"
					+ "The process is: A participant rolls a speed within the given range of base speed +/- variability, then\n"
					+ "that speed is multiplied by this constant based on the section of track they are on\n"
					+ "This is how participants all go faster on the straights and slow down on the corners with the default values\n"
					+ "(but you can make some really strange races)\n"
					+ "NOTE: Extreme differences between these multipliers will cause participants to instantaneously speed up or slow down,\n"
					+ "this is because the acceleration algorithm assumes there is enough space to accelerate up to speed or decelerate (and high differences\n"
					+ "in multipliers can make it so that is impossible so it doesn't even try to accelerate/decelerate)"));
		}
	...
	private void setUpConfigPane() {
		...
		for (int i = 0; i < 6; i++) {
			ObservableList<TrackSpeed> options = FXCollections.observableArrayList();
		...
		myTrackSectionComboBoxes.get(0).getSelectionModel().select(TrackSpeed.SLOW);
		myTrackSectionComboBoxes.get(1).getSelectionModel().select(TrackSpeed.MEDIUM);
		myTrackSectionComboBoxes.get(2).getSelectionModel().select(TrackSpeed.FAST);
		myTrackSectionComboBoxes.get(3).getSelectionModel().select(TrackSpeed.SLOW);
		myTrackSectionComboBoxes.get(4).getSelectionModel().select(TrackSpeed.MEDIUM);
		myTrackSectionComboBoxes.get(5).getSelectionModel().select(TrackSpeed.FAST);
		...
		for (int i = 0; i < speedBracketList.size(); i++) {
			...
			trackSpeedConfigPane.add(new SpeedConfigDisplay(currentSpeed,
					(newSpeed, range) -> updateSpeedBracket(currentSpeed, newSpeed, range)), 0, i);
		}

		// Set up track section speed config pane
		trackSectionSpeedConfigPane.add(new Text("Slow Track Section Multiplier: "), 0, 0);
		trackSectionSpeedConfigPane.add(new Text("Medium Track Section Multiplier: "), 0, 1);
		trackSectionSpeedConfigPane.add(new Text("Fast Track Section Multiplier: "), 0, 2);
		...
		for (int i = 0; i < trackSpeedMultiplierFields.size(); i++) {
			trackSectionSpeedConfigPane.add(trackSpeedMultiplierFields.get(i), 1, i);
			trackSpeedMultiplierFields.get(i).setPrefColumnCount(4);
		}

		// Add to ConfigPane
		// Add a message about the tooltips
		Text tooltipMessage = new Text("Hover over Text Fields/Sliders to see help and usage tips.\n"
		...
		// First instantiate all the values in estimateTimes and add them to their own
		// pane
		for (int i = 0; i < ParticipantSpeed.values().length; i++) {
			Text estimate = new Text(
					"Estimated time for a " + ParticipantSpeed.values()[i] + " racer to complete a lap: "
							+ String.format("%.2f",
									myTrack.getTrackLength() / ParticipantSpeed.values()[i].getVelocity() / 1000)
							+ " seconds");
	...
	private void setUpGenerationControlPane() {
		...
		generationControlPane.getChildren().addAll(fileDisplay, outputFileButton, submitRace, progressBar);
...
//	}
	...
	private void updateSpeedBracket(ParticipantSpeed speedBracket, double newSpeed, double newRange) {
	...
	private void updateEstimate(ParticipantSpeed speedBracket) {
		estimateTimes.get(speedBracket).setText("Estimated time for a " + speedBracket + " racer to complete a lap: "
				+ String.format("%.2f", myTrack.getTrackLength() / speedBracket.getVelocity() / 1000) + " seconds");
	}
...
//	}
	...
	private void chooseFile() {
		...
		chooser.getExtensionFilters().add(new ExtensionFilter("Race File", "*.rce"));
...
//		System.out.println(raceActuallyOver);
	...
	private class TrackChangeListener implements ChangeListener<String> {
		...
		public void changed(ObservableValue<? extends String> observable, String oldValue, String newValue) {
			...
			if (validInteger(trackLengthField.textProperty().get()) && validInteger(xRatioField.textProperty().get())
					&& validInteger(yRatioField.textProperty().get())) {
				myTrack = new OvalTrack(Integer.parseInt(trackLengthField.textProperty().get()),
						Integer.parseInt(xRatioField.textProperty().get()),
						Integer.parseInt(yRatioField.textProperty().get()));

				// Update estimates for all speed brackets (only matters if track length
				// changed)
...
//						.collect(Collectors.toList());
				...
				for (int i = 0; i < 6; i++) {
					trackSpeeds.add(myTrackSectionComboBoxes.get(i).getSelectionModel().getSelectedItem());
				}
				myTrack.setSections(trackSpeeds);

				// Get multipliers from TextFields
				// This is bad, program will do nothing on submit if these fields are wrong.
				TrackSpeed.SLOW
						.setMultiplier(Double.parseDouble(trackSpeedMultiplierFields.get(0).textProperty().get()));
				TrackSpeed.MEDIUM
						.setMultiplier(Double.parseDouble(trackSpeedMultiplierFields.get(1).textProperty().get()));
				TrackSpeed.FAST
						.setMultiplier(Double.parseDouble(trackSpeedMultiplierFields.get(2).textProperty().get()));

				// The expected time is roughly the speed of the slowest participant.
				int expectedTime = myTrack.getTrackLength() / (int) ParticipantSpeed.SLOW.getVelocity();
				linesToWrite.clear();
				...
				Race race = new Race(myTrack, numLaps, telemetryInterval, participants);
